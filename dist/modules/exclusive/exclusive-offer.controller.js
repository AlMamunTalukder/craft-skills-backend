...
exports.exclusiveOfferController = void 0;
const catchAsync_1 = __importDefault(require("../../utils/catchAsync"));
const sendResponse_1 = __importDefault(require("../../utils/sendResponse"));
const config_1 = __importDefault(require("../../config"));
const exclusive_offer_service_1 = require("./exclusive-offer.service");
...
const paymentSuccess = async (req, res) => {
    try {
        ...
        let participant = await exclusive_offer_model_1.ExclusiveOfferParticipant.findOne({
        ...
        try {
            if (req.body.value_d) {
                const cleanStr = typeof req.body.value_d === 'string'
        ...
        try {
            await exclusive_offer_model_1.ExclusiveOfferParticipant.findOneAndUpdate({ transactionId: lookupTranId }, {
                $set: {
        ...
        try {
            const fresh = await exclusive_offer_model_1.ExclusiveOfferParticipant.findOne({
                transactionId: lookupTranId,
            });
            if (fresh)
                updatedParticipant = fresh;
        }
        catch (e) {
        ...
        try {
            (0, exclusive_visitor_controller_1.markVisitorRegistered)(req, res);
        }
        catch (visitorError) {
...
const ipn = async (req, res) => {
    ...
    try {
        ...
        // Queue Google Sheets
        await exclusive_offer_service_1.exclusiveOfferService
            .addToQueue({
...
const getParticipantById = (0, catchAsync_1.default)(async (req, res) => {
    ...
    const participant = await exclusive_offer_model_1.ExclusiveOfferParticipant.findById(id);
    if (!participant) {
        return (0, sendResponse_1.default)(res, {
            success: false,
            statusCode: 404,
...
const createParticipant = (0, catchAsync_1.default)(async (req, res) => {
    ...
    const participant = await exclusive_offer_model_1.ExclusiveOfferParticipant.create(payload);
    // ✅ Also add this participant to the batch's participants array
    if (participant.batchId) {
        await exclusive_batch_model_1.ExclusiveBatch.findByIdAndUpdate(participant.batchId, {
            $push: { participants: participant._id },
            $inc: { enrolledCount: 1 }, // ✅ Increment enrolled count
        });
    }
    try {
        await exclusive_offer_service_1.exclusiveOfferService.sendToGoogleSheets(participant);
    }
    catch (error) {
...
const updateParticipant = (0, catchAsync_1.default)(async (req, res) => {
    const id = req.params.id;
    // Get the existing participant to check if batch changed
    const existingParticipant = await exclusive_offer_model_1.ExclusiveOfferParticipant.findById(id);
    ...
    if (typeof req.body.whatsapp === 'string')
    ...
    const participant = await exclusive_offer_model_1.ExclusiveOfferParticipant.findByIdAndUpdate(id, updateData, {
        new: true,
        runValidators: true,
    });
    ...
    if (oldBatchId?.toString() !== newBatchId?.toString()) {
        ...
        if (oldBatchId) {
            await exclusive_batch_model_1.ExclusiveBatch.findByIdAndUpdate(oldBatchId, {
        ...
        // Add to new batch
        if (newBatchId) {
            await exclusive_batch_model_1.ExclusiveBatch.findByIdAndUpdate(newBatchId, {
                $push: { participants: participant._id },
                $inc: { enrolledCount: 1 },
            });
        }
...
const getParticipants = (0, catchAsync_1.default)(async (req, res) => {
    ...
    const [participants, total] = await Promise.all([
        exclusive_offer_model_1.ExclusiveOfferParticipant.find(filter)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limitNum)
            .populate('batchId', 'batchNo title'),
        exclusive_offer_model_1.ExclusiveOfferParticipant.countDocuments(filter),
    ]);
    (0, sendResponse_1.default)(res, {
        success: true,
        statusCode: 200,
...
const deleteParticipant = (0, catchAsync_1.default)(async (req, res) => {
    const id = req.params.id;
    ...
    await exclusive_offer_model_1.ExclusiveOfferParticipant.findByIdAndDelete(id);
    (0, sendResponse_1.default)(res, {
        success: true,
        statusCode: 200,
...
exports.exclusiveOfferController = {
    register,
    paymentSuccess,
    paymentFail,
    paymentCancel,
