...
logger.info('API Key: ' + (config.CLOUDINARY_CLOUD_KEY ? '***' + config.CLOUDINARY_CLOUD_KEY.slice(-4) : 'NOT SET'));
logger.info('API Secret: ' + (config.CLOUDINARY_CLOUD_SECRET ? '***' + config.CLOUDINARY_CLOUD_SECRET.slice(-4) : 'NOT SET'));
...
if (
    ...
    logger.error('Current values - API Secret: ' + (config.CLOUDINARY_CLOUD_SECRET ? 'SET' : 'NOT SET'));
}
